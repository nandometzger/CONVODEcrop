...
class CONVSTAR_unit(nn.Module):
	...
	def __init__(self, hidden_size, input_size,
		...
		if n_units==0:
			self.x_K = nn.Conv2d(input_size, hidden_size, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect', bias=bias)
			self.x_z = nn.Conv2d(input_size, hidden_size, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect', bias=bias)
			...
			self.x_K = nn.Sequential(
				nn.Conv2d(input_size, n_units, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect', bias=bias),
				nn.Tanh(),
				nn.Conv2d(n_units, hidden_size, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect', bias=bias))
			...
			self.h_K = nn.Sequential(
				nn.Conv2d(hidden_size, n_units, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect', bias=bias),
				nn.Tanh(),
				nn.Conv2d(n_units, hidden_size, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect', bias=bias))
		...
		if self.use_BN:
			self.bn_x_K = nn.BatchNorm2d(hidden_size)
			self.bn_x_z = nn.BatchNorm2d(hidden_size)
			self.bn_h_K = nn.BatchNorm2d(hidden_size)
	...
	def __init__(self, latent_dim, input_dim, 
		...
		if update_gate is None:
			self.update_gate = nn.Sequential(
			   nn.Conv2d(latent_dim * 2 + input_dim, n_units, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect'),
			   nn.Tanh(),
			   nn.Conv2d(n_units, latent_dim, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect'),
			   nn.Sigmoid())
		...
		if new_state_net is None:
			self.new_state_net = nn.Sequential(
			   nn.Conv2d(latent_dim * 2 + input_dim, n_units, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect'),
			   nn.Tanh(),
			   nn.Conv2d(n_units, latent_dim*2, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect'))
...
class CONVGRU_standard_unit(nn.Module):
	def __init__(self, latent_dim, input_dim, 
		...
		if update_gate is None:
			self.update_gate = nn.Sequential(
			   nn.Conv2d(latent_dim * 2 + input_dim, latent_dim, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect'),
			   nn.Sigmoid())
			utils.init_network_weights(self.update_gate)
		else: 
			self.update_gate  = update_gate
		...
		if new_state_net is None:
			self.new_state_net = nn.Sequential(
			   nn.Conv2d(latent_dim * 2 + input_dim, latent_dim*2, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect'),
			   )
			utils.init_network_weights(self.new_state_net)
		else: 
			self.new_state_net  = new_state_net
...
class CONVLSTM_unit(nn.Module):
	def __init__(self, hidden_size, input_size):
		...
		self.i2h = nn.Conv2d(input_size, 4 * self.hidden_size, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect')
		self.h2h = nn.Conv2d(self.hidden_size, 4 * self.hidden_size, kernel_size=kernel_size, stride=1, padding=padding, padding_mode='reflect')
